...
// Bind the calling thread to a specific logical CPU (core_id).
// Note: calling this from inside the thread and using pthread_self()
// is more robust than using another thread object's native_handle()
// because the std::thread object may not always be usable from
// within the thread start routine on every platform.
void bindThreadToCore(int core_id) {
  cpu_set_t cpuset;
  CPU_ZERO(&cpuset);
  CPU_SET(core_id, &cpuset);

  // Use pthread_self() to get the calling thread's handle.
  int result = pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpuset);
  if (result != 0) {
    std::cerr << "Error binding thread to core " << core_id
          << ", error code: " << result << std::endl;
  } else {
    std::cout << "Successfully bound thread to core " << core_id << std::endl;
  }
}
...
int main() {
  ...
  bindThreadToCore(0);
    for (int i = 0; i < generateLen; i++) {
      ...
      for (int m = 0; m < 32; m++) {
        ...
        const auto computeStart0 = std::chrono::high_resolution_clock::now();
        ...
        const auto computeEnd0 = std::chrono::high_resolution_clock::now();
        {
          std::lock_guard<std::mutex> lock(mutex0);
          subdone0 = true;
        }
        ...
        const auto computeStart1 = std::chrono::high_resolution_clock::now();
        ...
        _mlir_ciface_forward1(&sub3DMemRef0, &paramsContainers[m * 18 + 9],
                              &subResultContainer[0]);
        const auto computeEnd1 = std::chrono::high_resolution_clock::now();
        {
          std::lock_guard<std::mutex> lock(mutex0);
          subdone0 = true;
        }
        ...
        const auto computeStart2 = std::chrono::high_resolution_clock::now();
        _mlir_ciface_forward5(&resultMemRef2D0, &paramsContainers[m * 18 + 10],
                              &resultMemRef);
        const auto computeEnd2 = std::chrono::high_resolution_clock::now();
        {
          std::lock_guard<std::mutex> lock(mutex0);
          subdone0 = true;
        }
        ...
        const auto computeStart3 = std::chrono::high_resolution_clock::now();
        _mlir_ciface_forward3(&subResultContainer[0], &subResultContainer2D[0],
                              &subResultContainer[0]);
        const auto computeEnd3 = std::chrono::high_resolution_clock::now();
        const std::chrono::duration<double, std::milli> computeTime0 = computeEnd - computeStart;
        const std::chrono::duration<double, std::milli> computeTime1 = computeEnd0 - computeStart0;
        const std::chrono::duration<double, std::milli> computeTime2 = computeEnd1 - computeStart1;
        const std::chrono::duration<double, std::milli> computeTime3 = computeEnd2 - computeStart2;
        const std::chrono::duration<double, std::milli> computeTime4 = computeEnd3 - computeStart3;
        computeTime = computeTime + (computeTime0.count() / 1000.0) +
                      (computeTime1.count() / 1000.0) +
                      (computeTime2.count() / 1000.0) +
                      (computeTime3.count() / 1000.0) +
                      (computeTime4.count() / 1000.0);
      }
      {
        std::lock_guard<std::mutex> lock(mutex0);
        subdone0 = true;
  ...
  for (int i = 0; i < generateLen; i++) {
    ...
    const std::chrono::duration<double, std::milli> computeTime1 = computeEnd0 - computeStart0;
    const std::chrono::duration<double, std::milli> computeTime2 = computeEnd1 - computeStart1;
    computeTime = ((computeTime + computeTime7) / 2) + (computeTime1.count() / 1000.0) +
                      (computeTime2.count() / 1000.0);
    std::cout << "[log] Iteration " << i << " compute time: " << computeTime << "s." << std::endl;
